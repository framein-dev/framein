...
function runWithEnv(cwd, args, env) {
    try {
...
function runInput(cwd, args, input) {
    try {
...
test('cli task amend serializes concurrent writers without losing criteria', async () => {
    ...
    try {
        ...
        const results = await Promise.all(Array.from({ length: 6 }, (_, i) => runAsync(dir, ['task', 'amend', 'acceptance', `criterion ${i}`])));
        for (const r of results)
            assert.equal(r.code, 0, r.stderr + r.stdout);
        const show = run(dir, ['task', 'show']);
        for (let i = 0; i < 6; i++)
            assert.match(show.stdout, new RegExp(`criterion ${i}`));
    }
    finally {
        rmSync(dir, { recursive: true, force: true });
    }
...
test('cli risk includes untracked sensitive files after a baseline commit', () => {
    withTmp((dir) => {
        ...
        assert.deepEqual(j.hits.map((h) => h.file), ['src/auth/login.ts']);
    });
});
test('cli risk includes files before the first commit', () => {
    withTmp((dir) => {
        ...
        const j = JSON.parse(run(dir, ['risk', '--json']).stdout.trim());
        assert.equal(j.level, 'high');
        assert.ok(j.hits.some((h) => h.file === 'src/auth/login.ts'));
    });
});
...
test('cli verify --json returns the gate as structured JSON', () => {
    withTmp((dir) => {
        ...
        assert.ok(Array.isArray(j.checks) && j.checks.some((c) => c.label === 'Tests'));
    });
});
...
test('cli mcp serve answers JSON-RPC tools/list and tools/call over stdio', () => {
    withTmp((dir) => {
        ...
        const reqs = JSON.stringify({ jsonrpc: '2.0', id: 0, method: 'initialize', params: { protocolVersion: '2025-06-18' } }) + '\n' +
...
test('cli mcp register previews by default and writes (idempotently) with --write', () => {
    withTmp((dir) => {
        ...
        if (fr.command === 'node')
            assert.match(fr.args[0], /cli\.js$/); // node fallback points at this cli
