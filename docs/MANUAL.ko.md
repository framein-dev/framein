...
> **주 사용법 = 에이전트 구동.** `framein init`이 컨텍스트 파일(관리블록)에 사용 지침을 심어,
...
>
> Framein은 "여러 AI를 돌리는 실행기"나 상태 없는 일반 스킬팩이 아니라 **로컬 검증 게이트, 결정 원장,
> 네이티브 스킬 표면**입니다. 평소처럼 네이티브 CLI(Claude·Codex·Gemini)나 기존 스킬팩으로
...
아래 원칙은 공개 동작에서 유지해야 하는 설계 기준입니다.

| 원칙 | 의미 |
|---|---|
| **제어층 ≠ 실행기** | 터미널을 구동·스크래핑하지 않고, 공유 store/ledger로 **관측**한다. |
| **제자리(in-place) 기본** | 평소처럼 네이티브 CLI에 입력. 다른 역할은 한 세션 안으로 *불러온다*. 새 프롬프트 표면을 강제하지 않는다. |
| **no-relay** | 구독 자격증명을 중앙에 모으거나 대신 라우팅하지 않는다. 각 CLI가 자기 인증을 직접 처리한다. |
| **감사 cadence** | 매 턴 리뷰가 아니라 **게이트·이상징후**에서만 다른 모델을 부른다. |
| **fresh-on-read** | 공유는 pull 기반. "항상 최신을 조회 가능"이지 자동 푸시가 아니다. |
| **append-only decisions** | 결정은 수정·삭제 불가, 정정은 `supersede`. |
| **zero runtime dependency** | Node 내장만(`node:sqlite`/`node:test`) 사용한다. |
| **재사용 우선** | 기존 MCP·스킬을 재구현하지 않고 **감지·추천·등록**까지만 한다. |
...
**(2) 제품 루프 — 의도 → 검증 → 구조.** framein의 핵심 동선입니다.
...
```
 ...
 완료 게이트  framein verify / ship  (build·test 증거를 계약과 대조)
     ↓
 다른 모델이 blocker review  framein challenge  (주장만 반환, lead가 결정)
     ↓
...
| **`framein challenge "<제안>"`** | 다른 모델의 독립 반론 | 최대 2왕복, 미합의 시 사람에게 2선택지. `--run`으로 리뷰어 자동 |
...
모델 간 논쟁을 **형식 제한**(제안 → 반론 → 해결, **최대 2왕복**)합니다. 리뷰어는 코드를 고치지 않고
**주장·근거·요구만** 반환 → 주도권이 lead에게 유지.
```bash
framein challenge "<제안>"                       # 논쟁 시작 (lead 제안)
framein challenge "<제안>" --run                 # 리뷰어가 JSON verdict 생성 → Challenge로 ingest (라이브)
...
컴파일/투영**합니다. 다른 도구의 스킬을 교차 실행하거나 프록시하지 않습니다.
```bash
...
(인터랙티브 TUI를 프로그램으로 모는 PTY가 아닙니다).
```bash
...
framein mcp serve                 # MCP stdio 서버 실행 (NDJSON JSON-RPC — Content-Length 아님)
```
...
| Codex | `.codex/skills/fr-<동사>/SKILL.md` | `$fr-verify` (스킬 — `$` 접두어; Codex `/prompts`는 deprecated) |

대상 동사: `start · verify · ship · rescue · status · challenge`. 네임스페이스 `fr`로 묶어 베어 이름 충돌을
피합니다.
...
(project/lead/reviewer/task)를 보여줍니다.
```text
$ framein shell
┌─ FRAMEIN ─────────────────────────────┐
...
- **숨은 비용(정직 고지):** framein 화면과 라이브 TUI를 *동시에* 겹쳐 보여주려면 `node-pty`(네이티브 의존성)가
  필요합니다. zero-dep을 깨므로 **번들하지 않는 선택적 의존성**으로만 남깁니다.
...
[`README.md`](../README.md)와 이 매뉴얼을 함께 참고하세요.

---

**Framein by Frameout** · MIT([`LICENSE`](../LICENSE)) · 신뢰 경계 [`SECURITY.md`](../SECURITY.md).
