...
export type WrapHost = 'claude' | 'codex' | 'gemini';

// `run`: append --run so the wrapper actually executes the delegation (not just records intent).
// `independent`: append `--by <host>` so framein picks a reviewer model ≠ the calling agent — the whole
// point of an INDEPENDENT challenge (else codex would "challenge" codex). Resolved in cli.ts cmdChallenge.
export interface WrapVerb { verb: string; json: boolean; desc: string; run?: boolean; independent?: boolean; }
export const WRAP_VERBS: WrapVerb[] = [
...
export interface WrapperFile { path: string; content: string; }
...
const cmd = (bin: string, v: WrapVerb, host: WrapHost): string =>
  `${bin} ${v.verb}${v.json ? ' --json' : ''}${v.run ? ' --run' : ''}${v.independent ? ` --by ${host}` : ''}`;

export function genClaudeCommand(v: WrapVerb, bin = 'framein'): WrapperFile {
  const content = [
    '---',
    `description: ${v.desc}`,
...
export function genGeminiCommand(v: WrapVerb, bin = 'framein'): WrapperFile {
  const content = [
    `# ${PROVENANCE}`,
...
export function genCodexSkill(v: WrapVerb, bin = 'framein'): WrapperFile {
  const content = [
    '---',
    `name: fr-${v.verb}`,
    `description: ${JSON.stringify(v.desc)}`,
...
export function wrapperFiles(host: WrapHost, bin = 'framein'): WrapperFile[] {
