...
export interface DigestOptions { max?: number; }

export function buildAdrDigest(adrs: Adr[], opts: DigestOptions = {}): string {
  if (adrs.length === 0) return '_No decisions recorded yet._';
  const max = opts.max ?? 10;
  // Derived (append-only): an ADR is superseded only if a LATER one references it.
  const supersededIds = new Set(
    adrs.filter((a) => a.supersedes != null && a.id > a.supersedes).map((a) => a.supersedes as number),
  );
