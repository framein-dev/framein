...
import { PLAIN, statusTone, type Painter } from './ui/theme.js';

export interface TestSummary { passed: number; failed: number; }

export interface EvidenceBundle {
  build?: { command: string; exitCode: number };
  tests?: { command: string; summary: TestSummary | null; exitCode: number };
  changedFiles?: string[];
  unresolved?: string[];
}

export interface GateCheck { label: string; ok: boolean; detail?: string; }
export interface GateResult { ready: boolean; checks: GateCheck[]; warnings: string[]; }
 ...
 */
export function gate(contract: TaskContract | undefined, bundle: EvidenceBundle): GateResult {
  const checks: GateCheck[] = [];
  const warnings: string[] = [];

  if (bundle.build) checks.push({ label: 'Build', ok: bundle.build.exitCode === 0, detail: bundle.build.command });
  if (bundle.tests) {
  ...
  if (checks.length === 0) warnings.push('no build/test commands found — nothing was actually verified');

  if (contract) {
    for (const issue of contractIssues(contract)) warnings.push(`contract: ${issue}`);
    if (contract.acceptance.length) warnings.push(`${contract.acceptance.length} acceptance criteria need verification (reviewer/human)`);
  } else {
    warnings.push('no task contract — run `frame start <goal>` to define "done"');
  }
  for (const u of bundle.unresolved ?? []) warnings.push(`unresolved: ${u}`);

  return { ready: checks.length > 0 && checks.every((c) => c.ok), checks, warnings };
}

function header(r: GateResult): string {
  if (!r.ready) return 'NOT READY';
...
export function renderGate(r: GateResult, ui: Painter = PLAIN): string {
  const h = header(r);
  ...
  for (const w of r.warnings) lines.push(`${ui.tone(ui.sym.warn, 'warning')} ${w}`);
  return lines.join('\n');
}

/** Gate body + commit/deploy guidance (used by `frame ship`). */
export function renderShip(r: GateResult, ui: Painter = PLAIN): string {
  return [
    renderGate(r, ui),
    '',
