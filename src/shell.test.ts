...
import { routeShellLine, renderShellHelp, lobbyCompleter, LOBBY_PALETTE, type ShellState } from './shell.js';

const st = (): ShellState => ({ lead: 'claude' });
...
test('LOBBY_PALETTE: covers the key lobby commands, all slash-prefixed, with descriptions', () => {
  ...
  for (const c of ['/init', '/verify', '/ship', '/lead', '/go', '/help', '/exit']) assert.ok(cmds.includes(c), `palette missing ${c}`);
