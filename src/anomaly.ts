...
export interface AnomalySignal {
  kind: 'repeated-edits' | 'repeated-failure' | 'no-progress';
  target?: string;
  count: number;
  message: string;
}
...
export interface AnomalyOptions {
  repeatedEdits?: number;     // same file edited >= N times
  repeatedFailures?: number;  // same test failed >= N times
  noProgressTurns?: number;   // N trailing turns with no edit/commit
}

export function detectThrash(entries: LedgerEntry[], opts: AnomalyOptions = {}): AnomalySignal[] {
  ...
  for (const e of entries) {
    if (e.kind === 'edit' && e.target) editCounts.set(e.target, (editCounts.get(e.target) ?? 0) + 1);
    if (e.kind === 'test-fail' && e.target) failCounts.set(e.target, (failCounts.get(e.target) ?? 0) + 1);
  }
  for (const [target, count] of editCounts) {
    if (count >= editThreshold) signals.push({ kind: 'repeated-edits', target, count, message: `'${target}' edited ${count}× — possible thrash loop` });
  }
  for (const [target, count] of failCounts) {
    if (count >= failThreshold) signals.push({ kind: 'repeated-failure', target, count, message: `'${target}' failed ${count}× — stuck on the same test` });
  }
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (k === 'edit' || k === 'commit') break;
    if (k === 'turn') trailingTurns++;
  }
