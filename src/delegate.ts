...
// `args` are FIXED flags only (never the user's prompt); the prompt goes via `stdin`. This keeps
// the spawn injection-safe under shell:true (needed on Windows to resolve npm .cmd shims like
// codex) — args concatenated into a shell string are not escaped (Node DEP0190), so no user text
 ...
 */
export function buildInvocation(agent: Agent, prompt: string, opts: { trustFlags?: string[] } = {}): Invocation {
...
export function resolveAgent(roles: Partial<Record<Role, Agent>>, role: Role): Agent {
...
export function invocationCommand(inv: Invocation): string {
