...
import { ROLES, AGENTS, type StoreSnapshot, type Agent, type Role } from './types.js';
import { isAgent, isRole, selectAgent, DEFAULT_ROLE_PRIORITY } from './roles.js';
import { HANDOFF_START_PROMPT, buildInvocation, resolveAgent, renderInvocation, invocationCommand, interactiveCommand, type Invocation } from './delegate.js';
import { detectQuotaSignal } from './quota.js';
import { trustPlan, parseDuration, DEFAULT_TRUST_TTL_SEC } from './trust.js';
import { emptyContract, amendContract, contractIssues, renderContractFull, buildGuidedContract, GUIDED_CONTRACT_STEPS, type ContractField } from './task.js';
import { gate, renderGate, renderShip, parseTestSummary, type EvidenceBundle } from './evidence.js';
import { buildRescue, renderRescue, type GreenCheckpoint } from './rescue.js';
import { buildCapsule, renderCapsule, type Capsule } from './capsule.js';
import { debateStatus, newDebate, renderDebate, type Debate } from './disagree.js';
import {
  buildLeadResponsePrompt,
  buildReviewerPrompt,
  challengeFromVerdict,
  normalizeLeadModelResponse,
  normalizeReviewerVerdict,
  renderDecisionBrief,
  responseFromLeadModel,
  type ChallengeFacts,
  type LeadModelResponse,
  type ReviewerVerdict,
} from './challenge.js';
import { extractJson } from './ingest.js';
import { assessBlastRadius, renderBlast, riskTransition, type RiskLevel } from './blast.js';
...
import { wrapperFiles, WRAP_VERBS, PROVENANCE, type WrapHost } from './wrappers.js';
import { routeShellLine, renderShellHelp, handoffCardRows, lobbyCompleter, LOBBY_PALETTE, type ShellState, type ShellAction } from './shell.js';
import { initSelect, reduceSelectKey, renderSelectLines, type SelectItem, type KeyEvent } from './select.js';
import { initPalette, reducePaletteKey, renderPaletteSuggestions, paletteSuggestions } from './palette.js';
import { resolveCapabilities, type UiCapabilities } from './ui/capabilities.js';
import { painter, type Painter } from './ui/theme.js';
...
function rel(p: string): string { return p.replace(/^[.][/\\]/, ''); }
...
function openStore(): Store {
  if (!existsSync(DB_PATH)) fail('No .frame/store.db found. Run `framein init` first.');
...
function withCliWriteLock<T>(store: Store, scope: string, fn: () => T): T {
  ...
  try { return fn(); } finally { store.releaseLock(holder, { scope }); }
}

function ensureFrameDirIgnored(dir = '.'): boolean {
...
function parseId(raw: string | undefined, usage: string): number {
  const n = Number(raw);
  if (!raw || !Number.isInteger(n) || n <= 0) fail(usage);
  return n;
}
...
function cmdInit(opts: { lead?: Agent; quiet?: boolean } = {}): number {
  ...
  try {
    ...
    if (opts.lead) store.setRole('implementer', opts.lead); // first-run wizard's chosen lead, in one step
    ...
    let n = 0; for (const h of hosts) n += writeWrappers(h);
    if (!opts.quiet) {
      ...
      if (ignoredFrameDir) console.log('Ignored local cache: .frame/ (use `framein export` for the git-canonical snapshot).');
      if (hosts.length) console.log(`Installed ${n} host wrapper(s) for ${hosts.join(', ')} — /fr:verify (Claude/Gemini) · $fr-verify (Codex).`);
    }
    return n;
  } finally {
    store.close();
...
function writeWrappers(host: WrapHost): number {
  let n = 0;
  for (const f of wrapperFiles(host, WRAPPER_BIN)) { mkdirSync(dirname(f.path), { recursive: true }); writeFileSync(f.path, f.content); n++; }
  return n;
}
...
function cmdRules(args: string[]): void {
  ...
  withStore((store) => {
    if (sub === 'set' || sub === 'reset') {
      ...
      if (sub === 'reset') {
        ...
        const piped = !inline && !process.stdin.isTTY ? (() => { try { return readFileSync(0, 'utf8').trim(); } catch { return ''; } })() : '';
        text = (inline || piped).replace(/\\n/g, '\n'); // allow \n escapes in a one-line arg
        if (!text) fail('Usage: framein rules set "<text>"   (use \\n for line breaks, or pipe the text on stdin)');
      }
    ...
    if (sub !== 'show') fail("Unknown 'rules' subcommand. Use: show | set <text> | reset");
    const raw = store.getConfig('rules');
    const cur = (typeof raw === 'string' ? raw : '').trim();
    if (wantsJson(args)) { emitJson('rules', { rules: cur }); return; }
...
function cliCaps(): UiCapabilities {
  const out = process.stdout as Partial<{ getColorDepth(): number; columns: number }>;
...
function cliUi(): Painter { return painter(cliCaps()); }

// `--json`: stable machine output for wrappers/automation (ADR-0010). schemaVersion + command.
function wantsJson(args: string[]): boolean { return args.includes('--json'); }
...
function cmdRole(args: string[]): void {
  withStore((store) => {
    ...
    if (sub === 'set') {
      const role = args[1];
      const agent = args[2];
      if (!role || !agent) fail('Usage: framein role set <role> <agent>');
      if (!isRole(role)) fail(`Unknown role '${role}'. Valid: ${ROLES.join(', ')}`);
      if (!isAgent(agent)) fail(`Unknown agent '${agent}'. Valid: ${AGENTS.join(', ')}`);
      store.setRole(role, agent);
      writeNativeFiles('.', store.getState());
      console.log(`Set ${role} -> ${agent} (native files re-synced)`);
    } else if (sub === undefined || sub === 'list') {
      const roles = store.getRoles();
      if (Object.keys(roles).length === 0) console.log('  (no roles assigned)');
      for (const [r, a] of Object.entries(roles)) console.log(`  ${r} -> ${a}`);
    } else {
...
function cmdAdr(args: string[]): void {
  withStore((store) => {
    const sub = args[0];
    if (sub === 'add') {
      const title = args.slice(1).join(' ').trim();
      if (!title) fail('Usage: framein adr add <title>');
      ...
      if (!title) fail('Usage: framein adr supersede <id> <title>');
      if (!store.getAdr(oldId)) fail(`ADR-${oldId} not found`);
      if (store.isSuperseded(oldId)) fail(`ADR-${oldId} is already superseded`);
      ...
      if (adr.supersedes != null) console.log(`  supersedes  : ADR-${adr.supersedes}`);
      if (adr.authorAgent) console.log(`  author      : ${adr.authorAgent}`);
      if (adr.context) console.log(`  context     : ${adr.context}`);
      console.log(`  decision    : ${adr.decision}`);
      if (adr.consequences) console.log(`  consequences: ${adr.consequences}`);
    } else if (sub === undefined || sub === 'list') {
      const adrs = store.listAdrs();
      if (adrs.length === 0) console.log('  (no decisions recorded)');
      for (const a of adrs) {
...
function cmdSync(args: string[]): void {
  withStore((store) => {
    if (args.includes('--dry-run')) {
      ...
      if (written.length === 0) console.log('Already in sync (no changes).');
      else console.log(`Synced from source of truth: ${written.map(rel).join(', ')}`);
    }
  });
}

function cmdUnlock(args: string[]): void {
  withStore((store) => {
...
function cmdImport(args: string[]): void {
  ...
  if (!existsSync(src)) fail(`Snapshot not found: ${src}`);
  let snap: StoreSnapshot;
  try { snap = JSON.parse(readFileSync(src, 'utf8')) as StoreSnapshot; }
  catch { return fail(`Not valid JSON: ${src}`); }
  mkdirSync(FRAME_DIR, { recursive: true });
  const store = Store.open(DB_PATH);
  try {
    ...
    fail(`Import failed: ${(e as Error).message}`);
  } finally {
    store.close();
  }
}
...
function cmdAsk(args: string[]): void {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (VALUE_FLAGS.has(a)) { flags.push(a); i++; continue; } // skip the value so it can't leak into the prompt
    if (a.startsWith('--')) { flags.push(a); continue; }
    positional.push(a);
  }
  const role = positional[0];
  if (!role || !isRole(role)) fail(`Usage: framein ask <role> [prompt] [--show|--run]   (role: ${ROLES.join('|')})`);
  ...
  if (!prompt && !interactive) fail('No prompt. Pipe it on stdin or pass it after the role.');
  ...
  withStore((store) => {
    ...
    if (interactive) { // zero-dep human-in-the-loop attach: hand the agent's own TUI the terminal
      ...
      if (flags.includes('--show')) { console.log(`would attach to ${agent} interactively: ${cmd} (stdio:inherit)`); return; }
      ...
      if (res.error) fail(`Failed to launch ${agent}: ${(res.error as Error).message}`);
      return;
    }
    let trustFlags: string[] | undefined;
    ...
    if (useTrust) store.appendLedger('trust', agent, trustFlags?.join(' ') ?? '');
    if (flags.includes('--run')) { runDelegated(store, role, agent, inv); return; }
    ...
    if (signals.length) {
      ...
      for (const s of signals) console.log(`    - ${s.message}`);
    }
  });
}
 ...
 */
function runDelegated(store: Store, role: Role, agent: Agent, inv: Invocation): void {
  ...
  if (res.error) {
    store.appendLedger('delegate-fail', `${role}:${agent}`, (res.error as Error).message);
  ...
  if (!ok) process.exitCode = res.status ?? 1;
}

function cmdAudit(): void {
  withStore((store) => {
    const signals = detectThrash(store.listLedger());
    if (signals.length === 0) { console.log('No anomaly signals. (audit is blocker-only by default — ADR-0005)'); return; }
    console.log('Audit signals (consider pulling in the reviewer):');
    for (const s of signals) console.log(`  - [${s.kind}] ${s.message}`);
  });
}
...
function cmdLedger(args: string[]): void {
  withStore((store) => {
    ...
    if (sub === 'add') {
      const kind = args[1];
      if (!kind) fail('Usage: framein ledger add <kind> [target] [detail]');
      ...
      for (const e of entries) console.log(`  ${e.kind}${e.target ? ' ' + e.target : ''}`);
    } else {
...
async function serveMcp(): Promise<void> {
  const store = openStore();
  try { await serve(store); } catch (e) { console.error(String(e)); process.exitCode = 1; } finally { store.close(); }
}

function cmdMcp(args: string[]): void {
  if (args[0] === 'serve') { void serveMcp(); return; }
  if (args[0] === 'register') {
  ...
  if (servers.length === 0) {
    ...
    for (const s of servers) console.log(`  [${s.agent}] ${s.name}${s.command ? ` -> ${s.command}` : ''}`);
    const conflicts = findConflicts(servers);
    if (conflicts.length) console.log(`  conflicts (same name across agents): ${conflicts.join(', ')}`);
  }
...
function cmdSkills(): void {
  ...
  for (const s of FRAMEIN_SKILLS) console.log(`  [framein] ${s.name} — ${s.description}`);
  const detected = detectSkillsFromDisk();
  if (detected.length) {
    console.log('Detected (reused from your CLIs):');
    for (const s of detected) console.log(`  [${s.source}] ${s.name}${s.description ? ` — ${s.description}` : ''}`);
  }
...
function readVersion(): string {
  ...
  const baked = (globalThis as { __FRAMEIN_VERSION__?: string }).__FRAMEIN_VERSION__;
  if (baked) return `framein ${baked}`;
  try {
    const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')) as { version?: string };
    return `framein ${pkg.version ?? '?'}`;
  } catch { return 'framein (version unknown)'; }
}
...
function pkgHasScript(name: string): boolean {
  try {
...
function runShell(command: string): { exitCode: number; output: string } {
  ...
  if (res.error) return { exitCode: -1, output: (res.error as Error).message };
...
function collectEvidence(): EvidenceBundle {
  const bundle: EvidenceBundle = {};
  if (pkgHasScript('build')) { const r = runShell('npm run build'); bundle.build = { command: 'npm run build', exitCode: r.exitCode }; }
  if (pkgHasScript('test')) { const r = runShell('npm test'); bundle.tests = { command: 'npm test', summary: parseTestSummary(r.output), exitCode: r.exitCode }; }
  const changed = gitChangedFiles();
  if (changed.length) bundle.changedFiles = changed;
  return bundle;
}
  ...
  withStore((store) => {
    withCliWriteLock(store, 'evidence', () => {
      ...
      if (blast.level !== 'low') {
        ...
        const t = riskTransition(store.getMemory<RiskLevel>('risk', 'last'), blast.level);
        if (t) console.log(t);
      }
      store.setMemory('risk', 'last', blast.level);
      if (!result.ready) process.exitCode = 1; // ship is the enforced gate; verify is informational
    });
  });
}
...
function cmdRoute(args: string[]): void {
  if (args[0] !== 'explain') fail('Usage: framein route explain [role]');
  const roleArg = args.slice(1).find((a) => !a.startsWith('--'));
  if (roleArg && !isRole(roleArg)) fail(`Unknown role '${roleArg}'. Valid: ${ROLES.join(', ')}`);
  const role = (roleArg as Role | undefined) ?? 'reviewer';
  withStore((store) => {
    const e = explainRoute(role, { authMode: {} }, computeRepoStats(store.listLedger()));
    if (wantsJson(args)) { emitJson('route', { role: e.role, agent: e.agent, reasons: e.reasons, alternative: e.alternative ?? null }); return; }
...
function cmdStats(args: string[] = []): void {
  withStore((store) => {
    ...
    if (wantsJson(args)) { emitJson('stats', { stats }); return; }
    console.log(renderStats(stats, cliUi()));
  });
}

// --- Vibe Debt Delta + Ownership Brief (F-LOOP-9/10) ---
function cmdDebt(args: string[] = []): void {
  const d = parseDiffDebt(gitDiff());
  if (wantsJson(args)) { emitJson('debt', { addedLines: d.addedLines, removedLines: d.removedLines, addedDeps: d.addedDeps, todos: d.todos }); return; }
  console.log(renderDebt(d, cliUi()));
}

function cmdExplain(args: string[] = []): void {
  withStore((store) => {
    const cp = store.getMemory<GreenCheckpoint>('checkpoint', 'last');
    const brief = {
    ...
    if (!args.includes('--run')) {
      if (wantsJson(args)) { emitJson('explain', { ...brief, skeleton }); return; }
    ...
    if (wantsJson(args)) { emitJson('explain', { ...brief, agent: explainer, ok: r.ok, text: r.text || skeleton }); return; }
    console.log(r.text || skeleton);
    if (r.ok && r.text) store.setMemory('brief', 'last', { agent: explainer, ts: new Date().toISOString() });
  });
}

// --- Frame Recipe (F-LOOP-8): vendor-neutral protocols, compiled to each CLI (static, no store) ---
function cmdRecipe(args: string[]): void {
  const sub = args[0] ?? 'list';
  if (sub === 'list') {
    console.log('Recipes (vendor-neutral task protocols, compiled to each CLI — not cross-executed):');
    for (const r of listRecipes()) console.log(`  ${r.name} — trigger: ${r.trigger}, ${r.steps.length} steps`);
    return;
  }
  if (sub === 'show') {
    const r = getRecipe(args[1] ?? '');
    if (!r) fail(`Unknown recipe '${args[1] ?? ''}'. Try: framein recipe list`);
  ...
  if (sub === 'compile') {
    ...
    if (!r) fail(`Unknown recipe '${args[1] ?? ''}'. Try: framein recipe list`);
    const agent = args[2];
    if (!agent || !isAgent(agent)) fail(`Usage: framein recipe compile <name> <${AGENTS.join('|')}>`);
...
function cmdRisk(args: string[] = []): void {
  withStore((store) => {
    ...
    const prev = store.getMemory<RiskLevel>('risk', 'last');
    store.setMemory('risk', 'last', a.level);
    if (wantsJson(args)) { emitJson('risk', { level: a.level, hits: a.hits, requiredGates: a.requiredGates }); return; }
...
function spawnAgentText(agent: Agent, prompt: string): { ok: boolean; text: string } {
  const inv = buildInvocation(agent, prompt);
  const res = spawnSync(invocationCommand(inv), { input: inv.stdin, encoding: 'utf8', shell: true });
  return { ok: res.status === 0 && !res.error, text: (res.stdout ?? '').trim() || (res.stderr ?? '').trim() };
}

function cmdRescue(args: string[] = []): void {
  withStore((store) => {
    const signals = detectThrash(store.listLedger());
    const cp = store.getMemory<GreenCheckpoint>('checkpoint', 'last');
    const reviewer = store.getRole('reviewer');
    const report = buildRescue(signals, { lastGreen: cp, reviewer });
    if (wantsJson(args)) { emitJson('rescue', { triggered: report.triggered, signals: report.signals, lastGreen: report.lastGreen ?? null, options: report.options }); return; }
    ...
    if (args.includes('--run') && report.triggered) { // option A, live: reviewer diagnoses (no edits)
      ...
      if (r.ok && r.text) store.setMemory('rescue', 'last', { agent: rev, diagnosis: r.text.slice(0, 500), ts: new Date().toISOString() });
    }
  });
}

function cmdCheckpoint(args: string[]): void {
  const label = args.filter((a) => !a.startsWith('--')).join(' ').trim();
  const sha = gitHead();
  if (!sha) fail('Not a git repo (or no commits). `framein checkpoint` records the current commit as a known-good state.');
  withStore((store) => {
    store.setMemory('checkpoint', 'last', { sha, label } satisfies GreenCheckpoint);
...
function cmdRewind(args: string[]): void {
  const force = args.includes('--force');
  withStore((store) => {
    const cp = store.getMemory<GreenCheckpoint>('checkpoint', 'last');
    if (!cp) fail('No checkpoint recorded. Run `framein checkpoint` at a known-good state first.');
    if (!force) {
    ...
    if (res.status !== 0) fail(`git reset failed: ${(res.stderr ?? '').trim()}`);
...
function gatherCapsule(store: Store): Capsule {
  ...
  const cp = store.getMemory<GreenCheckpoint>('checkpoint', 'last');
  const ev = store.getMemory<EvidenceBundle>('evidence', 'last');
  const del = store.getMemory<{ agent: string; ok: boolean }>('delegation', 'last');
  const handoff = store.getMemory<string>('handoff', 'next');
  const debate = store.getMemory<Debate>('debate', 'current');
...
function cmdResume(): void {
  withStore((store) => {
    const saved = store.getMemory<Capsule>('capsule', 'last');
...
function cmdCapsule(args: string[]): void {
  const sub = args[0] ?? 'show';
  if (sub === 'show') { withStore((store) => console.log(renderCapsule(gatherCapsule(store), cliUi()))); return; }
...
const WRAP_HOSTS: WrapHost[] = ['claude', 'codex', 'gemini'];

function resolveHosts(arg: string | undefined): WrapHost[] {
  if (!arg || arg === 'all' || arg.startsWith('--')) return WRAP_HOSTS;
  if (!WRAP_HOSTS.includes(arg as WrapHost)) fail(`Unknown host '${arg}'. Use: ${WRAP_HOSTS.join(' | ')} | all`);
  return [arg as WrapHost];
}

function cliInstalled(c: string): boolean {
  const r = spawnSync(c, ['--version'], { encoding: 'utf8', shell: true });
  return r.status === 0 && !r.error;
}

function legacyWrapperPaths(host: WrapHost): string[] {
  if (host !== 'codex') return [];
  return WRAP_VERBS.map((v) => `.codex/skills/fr-${v.verb}/SKILL.md`);
}

function cmdIntegrations(args: string[]): void {
  const sub = args[0] ?? 'list';
  if (sub === 'list') {
  ...
  if (sub === 'show') {
    for (const h of hosts) for (const f of wrapperFiles(h, WRAPPER_BIN)) { console.log(`# ${f.path}`); console.log(f.content); }
    return;
  }
  if (sub === 'install') {
    const write = args.includes('--write');
    let n = 0;
    for (const h of hosts) for (const f of wrapperFiles(h, WRAPPER_BIN)) {
      if (write) { mkdirSync(dirname(f.path), { recursive: true }); writeFileSync(f.path, f.content); console.log(`wrote ${f.path}`); }
      else console.log(`would write ${f.path}`);
      n++;
    }
    console.log(write
  ...
  if (sub === 'uninstall') {
    let n = 0;
    for (const h of hosts) for (const f of wrapperFiles(h)) {
      if (existsSync(f.path) && readFileSync(f.path, 'utf8').includes(PROVENANCE)) { rmSync(f.path); console.log(`removed ${f.path}`); n++; }
    }
    for (const h of hosts) for (const p of legacyWrapperPaths(h)) {
      if (existsSync(p) && readFileSync(p, 'utf8').includes(PROVENANCE)) { rmSync(p); console.log(`removed legacy ${p}`); n++; }
    }
...
function cmdDoctor(): void {
  ...
  for (const c of WRAP_HOSTS) console.log(`  CLI ${c.padEnd(7)} ${cliInstalled(c) ? 'installed' : 'not found'}`);
  for (const h of WRAP_HOSTS) {
    const present = wrapperFiles(h).filter((f) => existsSync(f.path)).length;
...
function cmdSetup(): void {
  ...
  console.log('');
  if (detected.length === 0) console.log('No agent CLI detected on PATH. Install claude/codex/gemini, then: framein integrations install all --write');
  else if (missing.length === 0) console.log('All detected agent wrappers are installed. Use /fr:verify (Claude/Gemini) or $fr-verify (Codex) inside your agent.');
  else console.log(`Next: framein integrations install ${missing.join(' ')} --write   (then use /fr:verify in your agent)`);
}
...
function agentAvailable(agent: Agent): boolean {
  const r = spawnSync(interactiveCommand(agent), ['--version'], { encoding: 'utf8', shell: true });
  return r.status === 0 && !r.error;
}

function launchLeadTui(state: ShellState, agent: Agent, interactive: boolean, prompt: string | undefined,
                       caps: UiCapabilities, pause: () => void, resume: () => void, initialPrompt?: string): void {
  const ui = painter(caps);
  state.lead = agent;
  if (prompt) console.log(ui.tone(`(note: a prompt isn't seeded into the native TUI — paste it once ${agent} opens)`, 'muted'));
  if (!interactive) { console.log(ui.tone(`(skipped launching ${agent}: the lobby needs a TTY to hand over the native UI)`, 'muted')); return; }
  ...
  if (initialPrompt) console.log(ui.tone(`  handoff prompt seeded; ${agent} should pull the capsule first.`, 'muted'));
  console.log(ui.tone(`→ handing the terminal to ${agent} — framein is paused.`, 'brand'));
  if (resumeSession) console.log(ui.tone(`  ↻ resuming your previous ${agent} session (continuity).`, 'muted'));
  if (trusted) console.log(ui.tone(`  ⚠ trust ON — ${agent} runs WITHOUT approval prompts (${trustFlags.join(' ')}).`, 'danger'));
  ...
  resume();
  if (res.error) console.error(ui.tone(`Could not launch ${agent}: ${res.error.message}`, 'danger'));

  // Return event + recap + auto-handoff: if the agent armed `capsule <next>` before exiting, switch to it.
  let nextLead: Agent | undefined;
  if (entered && existsSync(DB_PATH)) {
    try {
      const s = Store.open(DB_PATH);
      try {
        s.appendLedger('return', agent);
        const pend = s.getMemory<string>('handoff', 'next');
        if (pend) { s.deleteMemory('handoff', 'next'); if (isAgent(pend) && pend !== agent) nextLead = pend; } // one-shot
      } finally { s.close(); }
    } catch { /* ignore */ }
  }
...
function shellStatusRows(state: ShellState): [string, string][] {
  const branch = gitBranch();
  const rows: [string, string][] = [['project', `${basename(process.cwd())}${branch ? ` · ${branch}` : ''}`], ['lead', state.lead]];
  if (existsSync(DB_PATH)) {
    try {
      const store = Store.open(DB_PATH);
      try {
        const rev = store.getRole('reviewer');
        if (rev) rows.push(['reviewer', rev]);
...
function initialLead(): Agent {
  if (existsSync(DB_PATH)) {
    try {
      const store = Store.open(DB_PATH);
      try { const impl = store.getRole('implementer'); if (impl && isAgent(impl)) return impl; }
      finally { store.close(); }
...
function promptSelect(header: string, items: SelectItem[], caps: UiCapabilities, startIndex = 0, clearOnExit = false): Promise<string | null> {
  const ui = painter(caps);
  ...
  const draw = (first: boolean): void => {
    ...
    if (!first) { moveCursor(out, 0, -drawn); clearScreenDown(out); }
    out.write(lines.map((l, i) => (i === 0 ? ui.tone(l, 'muted') : l)).join('\n') + '\n');
    drawn = lines.length;
  };

  return new Promise<string | null>((resolve) => {
    const restore = (): void => { try { if (process.stdin.isTTY) process.stdin.setRawMode(false); out.write('\x1b[?25h'); } catch { /* best effort */ } };
    ...
    const cleanup = (result: string | null): void => {
      ...
      if (clearOnExit && drawn) { moveCursor(out, 0, -drawn); clearScreenDown(out); } // erase the menu
      restore();
      resolve(result);
    };
    const onKey = (_str: string, key: KeyEvent): void => {
      const step = reduceSelectKey(state, key);
      if (step.kind === 'cancel') return cleanup(null);
      if (step.kind === 'accept') return cleanup(step.value);
      state = step.state;
      draw(false);
    };
...
function readLobbyLine(state: ShellState, caps: UiCapabilities): Promise<{ kind: 'line'; line: string } | { kind: 'exit' }> {
  const ui = painter(caps);
  const out = process.stdout;
  ...
  return new Promise((resolve) => {
    const restore = (): void => { try { if (process.stdin.isTTY) process.stdin.setRawMode(false); } catch { /* best effort */ } };
    const fit = (s: string): string => (s.length > cols - 1 ? `${s.slice(0, cols - 2)}…` : s);
    ...
    const shownLine = (): string => {
      if (!ps.navigated) return ps.buf;
      const sugg = paletteSuggestions(ps.buf, LOBBY_PALETTE);
      return sugg.length ? sugg[Math.min(ps.index, sugg.length - 1)].cmd : ps.buf;
    };
    const draw = (first: boolean): void => {
      if (!first) { out.write('\r'); clearScreenDown(out); }      // cursor is at end of input → col0 + wipe down
      ...
      if (note) lines.push(ui.tone(fit(note), 'muted'));
      for (const ln of lines) out.write(`\n${ln}`);
      below = lines.length;
      if (below > 0) { moveCursor(out, 0, -below); out.write('\r'); moveCursor(out, promptPlain.length + shown.length, 0); }
    };
    const finish = (result: { kind: 'line'; line: string } | { kind: 'exit' }): void => {
      out.write('\r'); clearScreenDown(out);                      // erase the live menu
    ...
    const onKey = (_str: string | undefined, key: KeyEvent): void => {
      ...
      if (step.kind === 'submit') return finish({ kind: 'line', line: step.line });
      if (step.kind === 'exit') return finish({ kind: 'exit' });
      if (step.kind === 'sigint') {
        if (ps.buf) { ps = initPalette(''); note = ''; sigintArmed = false; return draw(false); } // first Ctrl-C clears the line
        if (sigintArmed) return finish({ kind: 'exit' });
        sigintArmed = true; note = "press Ctrl-C again (or /exit) to leave the lobby"; return draw(false);
      }
      ps = step.state; note = ''; sigintArmed = false; draw(false);
    };
...
function promptLeadSelect(current: Agent, caps: UiCapabilities): Promise<Agent | null> {
  ...
  return promptSelect('pick a lead   ↑↓ move · type to filter · enter select · esc cancel', items, caps, ci) as Promise<Agent | null>;
}
...
async function firstRunWizard(caps: UiCapabilities): Promise<Agent | null> {
  const ui = painter(caps);
  ...
  const items: SelectItem[] = installed.map((a) => ({ value: a, label: a, hint: 'installed' }));
  ...
  return await promptSelect('First run — pick your lead agent   ↑↓ · enter · esc to skip', items, caps, 0, true) as Agent | null;
}
...
function projectHasFiles(): boolean {
  try { return readdirSync('.').some((f) => f !== '.frame'); } catch { return false; }
}
...
async function confirmInitInExistingProject(caps: UiCapabilities): Promise<boolean> {
  const ui = painter(caps);
...
async function offerWrappers(agent: Agent, caps: UiCapabilities): Promise<void> {
  if (!cliInstalled(agent)) return;                                 // CLI not installed → nothing to wrap
  if (wrapperFiles(agent).every((f) => existsSync(f.path))) return; // already installed
  const ui = painter(caps);
  const ans = await promptSelect(`Install ${agent} commands (${agent === 'codex' ? '$fr-*' : '/fr:*'})?`,
    [{ value: 'yes', label: 'yes', hint: 'install now' }, { value: 'no', label: 'no', hint: 'later' }], caps, 0, true);
  if (ans === 'yes') console.log(ui.tone(`installed ${writeWrappers(agent)} ${agent} wrapper(s)`, 'brand'));
}
...
function applyLobbyCommon(action: ShellAction, state: ShellState, caps: UiCapabilities, interactive: boolean): boolean {
  const ui = painter(caps);
  switch (action.kind) {
    case 'noop': return true;
    case 'help': console.log(renderShellHelp()); return true;
    case 'setLead':
    ...
    case 'error': console.error(ui.tone(action.message, 'danger')); return true;
    case 'engine':
      if (action.args[0] === 'mcp' && action.args[1] === 'serve') { console.error('`mcp serve` would consume the lobby stdin — run it as `framein mcp serve` instead.'); return true; }
      try { runCommand(action.args); }
      catch (e) { if (e instanceof CliError) { if (e.message) console.error(e.message); } else throw e; }
...
function cmdShell(): void {
  ...
  void (async (): Promise<void> => {
    ...
    if (firstRun) {
      ...
      if (chosen) {
        ...
        if (proceed) {
          ...
          } catch (e) { if (e instanceof CliError && e.message) console.error(e.message); }
        } else {
    ...
    if (interactive) {
      ...
      console.log('');
      if (setupNote) console.log(setupNote);
    ...
    const open = (): void => {
      ...
      const next = () => { if (interactive) { iface.setPrompt(`${ui.tone('fr', 'brand')}(${state.lead})${arrow} `); iface.prompt(); } };
      iface.on('line', (line) => {
        sigintArmed = false;
        const action = routeShellLine(line, state);
        if (applyLobbyCommon(action, state, caps, interactive)) { next(); return; }
        switch (action.kind) {
          case 'exit': iface.close(); return; // close handler prints bye
          case 'pickLead': console.log(`lead: ${state.lead}`); break; // non-TTY fallback: never block on a picker
          case 'launchLead': launchLeadTui(state, action.agent, interactive, action.prompt, caps, () => iface.pause(), () => iface.resume()); break;
        }
        next();
      });
      iface.on('SIGINT', () => {
        if (sigintArmed) { iface.close(); return; }
        sigintArmed = true;
    ...
    const lobbyLoop = async (): Promise<void> => {
      for (;;) {
        ...
        if (action.kind === 'pickLead') {
          ...
          } else console.log(ui.tone(`lead: ${state.lead}`, 'muted'));
    ...
    if (interactive) await lobbyLoop();
    else open();
  })();
}

const CONTRACT_FIELDS: ContractField[] = ['goal', 'preserve', 'acceptance', 'protected', 'nongoal'];
...
function surfaceContractChange(what: string): void {
...
async function runGuidedStart(): Promise<void> {
  const ui = painter(cliCaps());
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  const ask = (q: string): Promise<string> => new Promise((res) => rl.question(`${ui.tone('?', 'brand')} ${q}\n  `, res));
  console.log(ui.tone('Guided start — define the contract (the definition of done). Empty goal aborts.', 'muted'));
  const answers: Partial<Record<ContractField, string>> = {};
  for (const step of GUIDED_CONTRACT_STEPS) {
    const a = (await ask(step.question)).trim();
    if (step.field === 'goal' && !a) { console.log(ui.tone('No goal — aborted.', 'muted')); rl.close(); return; }
    if (a) answers[step.field] = a;
  }
  rl.close();
  ...
  withStore((store) => {
    withCliWriteLock(store, 'task', () => {
      ...
      for (const i of contractIssues(c)) console.log(`  ⚠ ${i}`);
...
function cmdTask(args: string[]): void {
  if ((args[0] ?? 'show') === 'start' && !args.slice(1).join(' ').trim() && Boolean(process.stdin.isTTY) && !inLobby) {
    void runGuidedStart(); return; // conversational contract builder (terminal only)
  }
  withStore((store) => {
    const sub = args[0] ?? 'show';
    if (sub === 'start') {
      const goal = args.slice(1).join(' ').trim();
      if (!goal) fail('Usage: framein task start <goal>');
      withCliWriteLock(store, 'task', () => {
      ...
      if (!field || !CONTRACT_FIELDS.includes(field as ContractField) || !value) {
        fail(`Usage: framein task amend <${CONTRACT_FIELDS.join('|')}> <value>`);
      }
      withCliWriteLock(store, 'task', () => {
        const cur = store.getTaskContract();
        if (!cur) fail('No active task contract. Run `framein task start <goal>` first.');
        store.setTaskContract(amendContract(cur, field as ContractField, value));
        writeNativeFiles('.', store.getState());
      ...
      const cur = store.getTaskContract();
      if (wantsJson(args)) { emitJson('task', { contract: cur ?? null, issues: cur ? contractIssues(cur) : [] }); return; }
      if (!cur) { console.log('No active task contract. Run `framein task start <goal>`.'); return; }
      console.log(renderContractFull(cur, cliUi()));
      for (const i of contractIssues(cur)) console.log(`  ⚠ ${i}`);
    } else {
...
function cmdChallenge(args: string[]): void {
  withStore((store) => {
    const reviewer = store.getRole('reviewer');
    let d = store.getMemory<Debate>('debate', 'current');

    if (args.includes('--show')) { if (!d) { console.log('No open debate.'); return; } console.log(renderDebate(d, cliUi())); return; }
    if (args.includes('--accept')) {
      if (!d) fail('No open debate to accept.');
      d.entries.push({ kind: 'challenge', challenge: { verdict: 'accept', by: reviewer } });
      store.setMemory('debate', 'current', d); store.appendLedger('challenge', 'accept'); console.log(renderDebate(d, cliUi())); return;
    }
    if (args.includes('--block')) {
      if (!d) fail('No open debate. Start one: framein challenge "<proposal>".');
      ...
      store.setMemory('debate', 'current', d); store.appendLedger('challenge', '', claim.slice(0, 80)); console.log(renderDebate(d, cliUi())); return;
    }
    ...
    const positional: string[] = [];
    for (let i = 0; i < args.length; i++) {
      if (args[i] === '--by') { i++; continue; }
      if (args[i].startsWith('--')) continue;
      positional.push(args[i]);
    }
    const text = positional.join(' ').trim();
    if (!text) fail('Usage: framein challenge "<proposal>" [--run] | --block "<claim>" [--require "<change>"] | --accept | --show');
    ...
    if (args.includes('--run')) {
      ...
      if (by && reviewer === by) console.log(`(reviewer role is ${by} = the calling model — using ${indep} instead so the verdict is genuinely independent.)`);
...
function gatherChallengeFacts(store: Store, proposal: string, debate?: Debate): ChallengeFacts {
  const changed = gitChangedFiles();
  return {
    proposal,
    contract: store.getTaskContract(),
    capsule: gatherCapsule(store),
    evidence: store.getMemory<EvidenceBundle>('evidence', 'last'),
...
function independentReviewer(store: Store, by: string | undefined): Agent | undefined {
  const configured = store.getRole('reviewer');
  if (configured && configured !== by && agentAvailable(configured)) return configured;
 ...
 */
function runReviewerChallenge(store: Store, d: Debate, reviewer: Agent, facts: ChallengeFacts): ReviewerVerdict | undefined {
...
function runLeadChallengeResponse(
  store: Store,
  d: Debate,
  lead: Agent,
  facts: ChallengeFacts,
  verdict: ReviewerVerdict,
): LeadModelResponse | undefined {
...
function cmdDecide(args: string[]): void {
  withStore((store) => {
    const d = store.getMemory<Debate>('debate', 'current');
    if (!d) fail('No open debate. Start one with `framein challenge "<proposal>"`.');
    const verb = args[0];
    const text = args.slice(1).join(' ').trim();
    if (verb !== 'accept' && verb !== 'reject') fail('Usage: framein decide accept|reject [text]');
...
function cmdTrust(args: string[]): void {
  ...
  if (!agent || !isAgent(agent)) fail(`Usage: framein trust <agent> [--ttl <dur>]   (agent: ${AGENTS.join('|')})`);
  let ttlSec: number | undefined;
  const ttlIdx = args.indexOf('--ttl');
  if (ttlIdx !== -1) {
    const parsed = args[ttlIdx + 1] ? parseDuration(args[ttlIdx + 1]) : null;
    if (parsed === null) fail('Usage: framein trust <agent> --ttl <dur>   (e.g. 30m, 1h, 90s)');
    ttlSec = parsed;
  }
...
function printHelp(): void {
  ...
  for (const [c, d] of HELP_ROWS) console.log(`  framein ${c.padEnd(w)}  ${d}`);
}
...
function runCommand(argv: string[]): void {
  const [cmd, ...rest] = argv;
  {
    ...
    if (cmd && USAGE[cmd] && rest.includes('--help')) { console.log(USAGE[cmd]); return; }
    switch (cmd) {
      ...
      case 'unlock': cmdUnlock(rest); break;
      ...
      case 'ledger': cmdLedger(rest); break;
      case 'trust': cmdTrust(rest); break;
      case 'task': cmdTask(rest); break;
      case 'start': cmdTask(['start', ...rest]); break; // front-stage verb: start a Task Contract
      case 'verify': cmdVerify(rest); break;
      case 'ship': cmdShip(rest); break;
      case 'risk': cmdRisk(rest); break;
      case 'route': cmdRoute(rest); break;
      case 'stats': cmdStats(rest); break;
      case 'recipe': cmdRecipe(rest); break;
      case 'debt': cmdDebt(rest); break;
      case 'explain': cmdExplain(rest); break;
      case 'rescue': cmdRescue(rest); break;
      case 'checkpoint': cmdCheckpoint(rest); break;
      case 'rewind': cmdRewind(rest); break;
      case 'pause': cmdPause(); break;
      case 'resume': cmdResume(); break;
      case 'capsule': cmdCapsule(rest); break;
      case 'challenge': cmdChallenge(rest); break;
      case 'decide': cmdDecide(rest); break;
      case 'integrations': cmdIntegrations(rest); break;
      case 'doctor': cmdDoctor(); break;
      case 'setup': cmdSetup(); break;
      case '-v':
      case '--version':
        console.log(readVersion()); break;
      case undefined:
        ...
        if (process.stdin.isTTY) { cmdShell(); break; }
        printHelp(); break;
      case '-h':
      case '--help':
        printHelp(); break;
      case 'lobby':
      case 'shell': cmdShell(); break; // `shell` kept as a hidden back-compat alias
      default:
        printHelp();
...
function main(): void {
  try {
    ...
    if (e instanceof CliError) { if (e.message) console.error(e.message); process.exit(1); }
    throw e;
  }
}
main();
