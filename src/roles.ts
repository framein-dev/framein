...
import type { Agent, AgentStats, AuthMode, Role } from './types.js';
import { AGENTS, ROLES } from './types.js';

/** Runtime guard: is `x` one of the three first-class agents? */
export function isAgent(x: string): x is Agent {
  return (AGENTS as readonly string[]).includes(x);
}
...
export const DEFAULT_ROLE_PRIORITY: Record<Role, Agent[]> = {
  lead:        ['claude', 'codex'],
...
export function isForbiddenAuth(agent: Agent, auth: AuthMode): boolean {
  return agent === 'gemini' && auth === 'consumer-login';
}

export interface RouteContext {
  role: Role;
  authMode: Partial<Record<Agent, AuthMode>>;
  remainingQuota?: Partial<Record<Agent, number>>;   // 0..1
  recentFailures?: Partial<Record<Agent, number>>;
  costBand?: Partial<Record<Agent, number>>;         // 0..1 (higher = pricier)
  unavailable?: Partial<Record<Agent, boolean>>;     // hard failover (e.g. quota exhausted, F-ROLE-3)
  repoStats?: Partial<Record<Agent, AgentStats>>;    // repo-local performance (F-LOOP-7)
  rolePriority?: Record<Role, Agent[]>;
}

/** Repo-local routing bonus: reward local success, penalize local quota trouble (F-LOOP-7). */
export function repoBonus(st: AgentStats | undefined): number {
  if (!st || st.delegations === 0) return 0;
...
export function scoreAgent(agent: Agent, ctx: RouteContext): number {
  const auth = ctx.authMode[agent];
  if (auth && isForbiddenAuth(agent, auth)) return -POLICY_PENALTY;
  if (ctx.unavailable?.[agent]) return -POLICY_PENALTY; // quota-exhausted / down => fail over to another agent
...
export function selectAgent(candidates: Agent[], ctx: RouteContext): Agent | null {
  let best: Agent | null = null;
  let bestScore = -POLICY_PENALTY;
  for (const a of candidates) {
    const s = scoreAgent(a, ctx);
    if (s === -POLICY_PENALTY) continue;
