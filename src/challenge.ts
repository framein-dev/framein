...
import type { Agent, LedgerEntry, TaskContract } from './types.js';
import type { EvidenceBundle } from './evidence.js';
import type { BlastAssessment } from './blast.js';
import type { Capsule } from './capsule.js';
import type { Challenge, Debate, LeadResponse } from './disagree.js';
...
const asStringArray = (x: unknown): string[] => {
  if (Array.isArray(x)) return x.map((v) => String(v).trim()).filter(Boolean).slice(0, 8);
  const s = asString(x);
  return s ? [s] : [];
};

export function normalizeReviewerVerdict(raw: Record<string, unknown> | null): ReviewerVerdict | null {
  if (!raw) return null;
  const verdict = asString(raw.verdict)?.toLowerCase();
  if (verdict !== 'challenge' && verdict !== 'accept') return null;
  return {
    verdict,
...
export function challengeFromVerdict(v: ReviewerVerdict, by?: Agent): Challenge {
  return {
    verdict: v.verdict,
    claim: v.claim,
...
export function normalizeLeadModelResponse(raw: Record<string, unknown> | null): LeadModelResponse | null {
  if (!raw) return null;
...
function renderEvidence(e: EvidenceBundle | undefined): string {
  if (!e) return 'No saved validation evidence. Ask for missing evidence if validation matters.';
  const lines: string[] = [];
  if (e.build) lines.push(`build: ${e.build.command} exit ${e.build.exitCode}`);
  if (e.tests) {
    const s = e.tests.summary;
    lines.push(`tests: ${e.tests.command} exit ${e.tests.exitCode}${s ? ` (${s.passed} passed, ${s.failed} failed)` : ''}`);
  }
  if (e.changedFiles?.length) lines.push(`changed_files: ${e.changedFiles.join(', ')}`);
  return lines.length ? lines.join('\n') : 'No build/test commands were recorded.';
}

function renderRisk(r: BlastAssessment | undefined): string {
  if (!r) return 'risk: unknown';
  const lines = [`risk: ${r.level}`];
  if (r.hits.length) lines.push(`risk_hits: ${r.hits.map((h) => `${h.category}:${h.file}`).join(', ')}`);
  if (r.requiredGates.length) lines.push(`required_gates: ${r.requiredGates.join(', ')}`);
  return lines.join('\n');
}

function renderCapsuleFacts(c: Capsule | undefined): string {
  if (!c) return 'No capsule available.';
  const lines = [`task: ${c.goal}`];
  if (c.contract) lines.push(`contract_digest: ${renderContractDigest(c.contract).replace(/\n/g, ' | ').replace(/\*\*/g, '')}`);
  lines.push(`next_action: ${c.nextAction}`);
  if (c.branch) lines.push(`branch: ${c.branch}`);
  if (c.changed.length) lines.push(`changed: ${c.changed.join(', ')}`);
  if (c.blocker) lines.push(`blocker: ${c.blocker}`);
  if (c.recentActivity.length) lines.push(`recent: ${c.recentActivity.join(', ')}`);
  return lines.join('\n');
}

function renderDebateFacts(d: Debate | undefined): string {
  if (!d) return 'No prior debate entries.';
  return d.entries.map((e) => {
    if (e.kind === 'proposal') return `proposal${e.proposal.by ? ` (${e.proposal.by})` : ''}: ${e.proposal.text}`;
    if (e.kind === 'challenge') return `challenge${e.challenge.by ? ` (${e.challenge.by})` : ''}: ${e.challenge.verdict}${e.challenge.claim ? ` - ${e.challenge.claim}` : ''}${e.challenge.requiredChange ? `; requires ${e.challenge.requiredChange}` : ''}`;
    if (e.kind === 'response') return `response${e.response.by ? ` (${e.response.by})` : ''}: ${e.response.text}`;
    return `decision: ${e.revision.accepted ? 'accept' : 'reject'} ${e.revision.text}`;
  }).join('\n');
}

export function buildReviewerPrompt(facts: ChallengeFacts): string {
  return [
...
export function buildLeadResponsePrompt(facts: ChallengeFacts, reviewer: ReviewerVerdict): string {
  return [
...
export function responseFromLeadModel(r: LeadModelResponse, by?: Agent): LeadResponse {
  return {
    text: r.text,
...
export function renderDecisionBrief(input: {
  ...
  leadResponse?: LeadModelResponse;
}): string {
  ...
  if (input.verdict.claim) lines.push(`claim: ${input.verdict.claim}`);
  if (input.verdict.requiredChange) lines.push(`required_change: ${input.verdict.requiredChange}`);
  if (input.verdict.basis.length) lines.push(`basis: ${input.verdict.basis.join(', ')}`);
  if (input.verdict.missingEvidence.length) lines.push(`missing_evidence: ${input.verdict.missingEvidence.join('; ')}`);
  if (input.leadResponse) {
    lines.push('');
    lines.push(`lead_response${input.lead ? ` (${input.lead})` : ''}: ${input.leadResponse.text}`);
    if (input.leadResponse.proposedRevision) lines.push(`proposed_revision: ${input.leadResponse.proposedRevision}`);
