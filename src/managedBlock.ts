...
function defangMarkerLines(core: string): string {
  return core.split('\n').map((ln) => {
    const t = ln.trim();
...
export function wrapManaged(coreBlock: string): string {
  return `${MANAGED_BEGIN}\n${defangMarkerLines(coreBlock)}\n${MANAGED_END}`;
}

/** All well-formed [begin,end] line-index regions, paired greedily. */
function findRegions(lines: string[]): Array<[number, number]> {
  const regions: Array<[number, number]> = [];
  let i = 0;
  while (i < lines.length) {
    if (isMarker(lines[i], MANAGED_BEGIN)) {
      let j = i + 1;
      while (j < lines.length && !isMarker(lines[j], MANAGED_END)) j++;
      if (j < lines.length) { regions.push([i, j]); i = j + 1; continue; }
    }
    i++;
  }
  return regions;
...
function markerLineCount(lines: string[]): number {
...
export function stripManagedBlocks(content: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (isMarker(lines[i], MANAGED_BEGIN)) {
      let j = i + 1;
      while (j < lines.length && !isMarker(lines[j], MANAGED_END)) j++;
      if (j < lines.length) { i = j; continue; } // drop a full region
      continue;                                   // dangling begin: drop just this line
    }
    if (isMarker(lines[i], MANAGED_END)) continue; // stray end: drop just this line
    out.push(lines[i]);
  }
  return out.join('\n');
}
 ...
 */
export function upsertManagedBlock(existing: string | null, title: string, managed: string): string {
