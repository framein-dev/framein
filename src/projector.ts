...
export function buildCoreBlock(state: ProjectState): string {
...
export function renderManagedBlock(state: ProjectState): string {
  return wrapManaged(buildCoreBlock(state));
}

/** A from-scratch native file (heading + managed block); used when no file exists yet. */
function projectFresh(title: string, state: ProjectState): string {
  return upsertManagedBlock(null, title, renderManagedBlock(state));
}

export function projectClaudeMd(state: ProjectState): string { return projectFresh('CLAUDE.md', state); }
export function projectAgentsMd(state: ProjectState): string { return projectFresh('AGENTS.md', state); }
export function projectGeminiMd(state: ProjectState): string { return projectFresh('GEMINI.md', state); }

export function projectAll(state: ProjectState): Record<'CLAUDE.md' | 'AGENTS.md' | 'GEMINI.md', string> {
  return {
