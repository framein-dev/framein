...
test('handleTool: append_adr then list/get', () => {
  const s = Store.open();
  const r = handleTool(s, 'append_adr', { title: 'Use X', decision: 'x' }) as { id: number };
  assert.equal(r.id, 1);
  assert.equal((handleTool(s, 'list_adr') as unknown[]).length, 1);
  assert.equal((handleTool(s, 'get_adr', { id: 1 }) as { title: string }).title, 'Use X');
  s.close();
});
...
test('dispatch: JSON-RPC tools/list and tools/call shapes', () => {
  const s = Store.open();
  const list = dispatch(s, { id: 1, method: 'tools/list' }) as { result: { tools: unknown[] } };
  assert.equal(list.result.tools.length, TOOLS.length);
  const call = dispatch(s, { id: 2, method: 'tools/call', params: { name: 'append_adr', arguments: { title: 'T' } } }) as { id: number; result: { content: { text: string }[] } };
  assert.equal(call.id, 2);
  assert.match(call.result.content[0].text, /"id":1/);
  const err = dispatch(s, { id: 3, method: 'bogus' }) as { error: { message: string } };
...
test('tools/list: every tool carries an object inputSchema (MCP requires it)', () => {
  const s = Store.open();
  const list = dispatch(s, { id: 1, method: 'tools/list' }) as { result: { tools: { name: string; inputSchema: { type: string; required?: string[] } }[] } };
  assert.equal(list.result.tools.length, TOOLS.length);
...
test('initialize: negotiates protocol version, string serverInfo.version, tools capability', () => {
  const s = Store.open();
  const ok = dispatch(s, { id: 1, method: 'initialize', params: { protocolVersion: '2024-11-05' } }) as { result: { protocolVersion: string; serverInfo: { version: unknown }; capabilities: { tools: { listChanged: boolean } } } };
  ...
  const fallback = dispatch(s, { id: 2, method: 'initialize', params: { protocolVersion: '1.0.0' } }) as { result: { protocolVersion: string } };
...
test('ping: replies with an empty result', () => {
  const s = Store.open();
  assert.deepEqual((dispatch(s, { id: 9, method: 'ping' }) as { result: unknown }).result, {});
  s.close();
});

test('tools/call: unknown tool / missing name are -32602; exec failure is isError (not a protocol error)', () => {
  const s = Store.open();
  const unknown = dispatch(s, { id: 1, method: 'tools/call', params: { name: 'nope', arguments: {} } }) as { error: { code: number } };
  assert.equal(unknown.error.code, -32602);
  const noName = dispatch(s, { id: 2, method: 'tools/call', params: { arguments: {} } }) as { error: { code: number } };
  assert.equal(noName.error.code, -32602);
  const failed = dispatch(s, { id: 3, method: 'tools/call', params: { name: 'append_adr', arguments: {} } }) as { result: { isError: boolean; content: { text: string }[] } };
  ...
  const ok = dispatch(s, { id: 4, method: 'tools/call', params: { name: 'append_adr', arguments: { title: 'T' } } }) as { result: { isError: boolean } };
...
test('dispatch: initialization ordering is enforced when a session is tracked', () => {
  ...
  const early = dispatch(s, { id: 1, method: 'tools/list' }, session) as { error: { code: number } };
  assert.equal(early.error.code, -32600); // request before initialize → rejected
  assert.deepEqual((dispatch(s, { id: 2, method: 'ping' }, session) as { result: unknown }).result, {}); // ping allowed pre-init
  ...
  const list = dispatch(s, { id: 4, method: 'tools/list' }, session) as { result: { tools: unknown[] } };
...
test('serve: NDJSON round-trip over streams + -32700 on a malformed line', async () => {
  ...
  const msgs = chunks.join('').trim().split('\n').map((l) => JSON.parse(l) as { id?: unknown; result?: { protocolVersion?: string; tools?: unknown[] }; error?: { code: number } });
