...
export interface ShellState { lead: Agent; trustUntil?: number; } // trustUntil: epoch ms while /go runs with bypass flags
...
export type ShellAction =
  ...
  | { kind: 'help' }
  | { kind: 'pickLead' }
  | { kind: 'setLead'; agent: Agent }
  | { kind: 'launchLead'; agent: Agent; prompt?: string }
  | { kind: 'toggleTrust' }
...
export function routeShellLine(line: string, state: ShellState): ShellAction {
  ...
  if (head === 'exit' || head === 'quit' || head === '/exit' || head === '/quit') return { kind: 'exit' };
  if (head === 'help' || head === '/help' || head === '?') return { kind: 'help' };

  if (head === '/lead') {
    if (rest.length === 0) return { kind: 'pickLead' }; // bare /lead → interactive picker (TTY); printed fallback otherwise
    const agent = rest[0].toLowerCase();
    if (!isAgent(agent)) return { kind: 'error', message: `Unknown agent '${rest[0]}'. Valid: ${AGENTS.join(', ')}` };
  ...
  if (head === '/trust') return { kind: 'toggleTrust' }; // arm/disarm permission-bypass for the next /go (time-boxed)
  // A bare agent name (optionally with a prompt) switches to + launches that agent's native TUI.
  if (isAgent(head)) {
    const prompt = rest.join(' ').trim();
    return { kind: 'launchLead', agent: head as Agent, prompt: prompt || undefined };
  }

  // Otherwise it's a framein command — `/verify` and `verify` both reach the engine.
  const verb = head.startsWith('/') ? head.slice(1) : head;
  if (verb === 'lobby' || verb === 'shell') return { kind: 'error', message: 'Already in the lobby.' };
...
export interface HandoffInfo {
  ...
  goal?: string;
  reviewer?: Agent;
  lastGreen?: string; // short sha (+ optional label)
  blocker?: string;
}
...
export function handoffCardRows(info: HandoffInfo): [string, string][] {
  const rows: [string, string][] = [['lead', info.lead]];
  if (info.reviewer) rows.push(['reviewer', info.reviewer]);
  rows.push(['task', info.goal && info.goal.trim() ? info.goal : 'no active contract']);
  if (info.lastGreen) rows.push(['last green', info.lastGreen]);
  if (info.blocker) rows.push(['blocker', info.blocker]);
  return rows;
}
...
export function lobbyCompleter(line: string): [string[], string] {
...
export const LOBBY_PALETTE: { cmd: string; desc: string }[] = [
