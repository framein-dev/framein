...
import { initPalette, paletteSuggestions, reducePaletteKey, renderPaletteSuggestions, type PaletteCmd, type PaletteState } from './palette.js';
...
const k = (name: string, extra: Partial<{ ctrl: boolean; sequence: string }> = {}) => ({ name, ...extra });
const type = (s: PaletteState, ch: string) => {
  const step = reducePaletteKey(s, { sequence: ch }, CMDS);
  assert.equal(step.kind, 'edit');
  return (step as { state: PaletteState }).state;
};
...
test('typing a printable char extends the buffer and resets the highlight', () => {
  const navigated: PaletteState = { buf: '/s', index: 2, navigated: true };
...
test('Enter runs exactly what was typed when the user has NOT navigated (no force-select)', () => {
  let s = initPalette('');
  for (const ch of '/verify') s = type(s, ch);
...
test('Enter runs the highlighted suggestion only after the user navigates with arrows', () => {
  ...
  s = (down as { state: PaletteState }).state;
...
test('Enter submits a bare verb / agent line verbatim (no slash, no popup)', () => {
  let s = initPalette('');
  for (const ch of 'codex fix the bug') s = type(s, ch);
...
test('Backspace removes the last char and resets navigation', () => {
  const navigated: PaletteState = { buf: '/ship', index: 0, navigated: true };
